...
imu_data_ml = np.load(os.path.join(results_path2, "IMU_data_ml.npy"), allow_pickle=True)
imu_data_all = np.load(os.path.join(results_path2, "IMU_data_all.npy"), allow_pickle=True)
...
sensor_array_length = ([entry[0]['acc'] for entry in imu_data_all])
...
for i, file in enumerate(os.listdir(path_extendedLeg)):
    if file.endswith('.xlsx') or file.endswith('.xls'):
        try:
            ...
            interval = np.arange(0, len(sensor_array_length[i]))
...
np.save(os.path.join(path_results, "labels_extendedLeg_IMU_100.npy"), np.array(labels_extendedLeg, dtype=object))  # Save files to results path
np.save(os.path.join(path_results, "labels_stiffMovement_IMU_100.npy"), np.array(labels_stiffMovement, dtype=object))
np.savetxt(
    os.path.join(path_results, "labels_extendedLeg_IMU_100.txt"),
...
np.savetxt(
    os.path.join(path_results, "labels_stiffMovement_IMU_100.txt"),
