...
def label_extract_cam(all_labels, behavior,interval):

    start_frames = np.round(pd.to_numeric(all_labels['Observation id'].str.split('_',expand=True)[5])/30*30)
    start_frames_single = np.sort(list(OrderedDict.fromkeys(start_frames)))
    # find start index for behavior
    behavior_start_single = np.round(all_labels['Start (s)'][all_labels['Behavior']==behavior]*30)
    behavior_start = (behavior_start_single+start_frames[all_labels['Behavior']==behavior]).sort_values()

    # find stop index for behavior
    behavior_stop_single = np.round(all_labels['Stop (s)'][all_labels['Behavior']==behavior]*30)
    behavior_stop = (behavior_stop_single+start_frames[all_labels['Behavior']==behavior]).sort_values()
    ...
    nrsf = 0
    for sf in start_frames_single:
        if sum(behavior_label_all[int(sf):int(sf+(5*30))])>=30/2:
            behavior_label_5s[nrsf]=1
        nrsf=nrsf+1
...
def label_extract_legrise_cam(all_labels, behavior,interval,start_frames_single):
    # find behavior start and stop index
    behavior_times = np.round(all_labels['Image index'][all_labels['Behavior']==behavior])/30*30
    # find start index for behavior
    behavior_start = behavior_times[0:-1:2]
