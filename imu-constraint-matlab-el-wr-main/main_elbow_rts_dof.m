...
%% quat angle
norms = sqrt(sum(el_rts_dof.^2, 2));
el_rts_dof_norm = el_rts_dof ./ norms;
angles_rad = 2 * acos(el_rts_dof_norm(:,1));
...
subplot(2,2,1);
plot(t, eul_el_rts_dof(:,1)); 
ylabel('rotation');
xticks(0:25:max(t));

subplot(2,2,2);
plot(t, eul_el_rts_dof(:,2)); 
ylabel('flexion'); 
xticks(0:25:max(t));

subplot(2,2,3);
plot(t, eul_el_rts_dof(:,3)); 
ylabel('deviation');
xticks(0:25:max(t));
